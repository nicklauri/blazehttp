...
impl H1Connection {
    ...
    pub async fn handle_connection(mut self) -> Result<()> {
        ...
        loop {
            ...
            let parsed_size = match req.parse_with_uninit_headers(reqbuf, &mut header) {
                Ok(Status::Complete(size)) => size,
                Ok(Status::Partial) => continue,
                Err(err) => Err(err).blaze_error()?,
            };

            // TODO: map httparse::Request into http::Request
...
fn map_to_http_request(hreq: &mut HttparseRequest) -> Result<Request> {
